...
async function scrapeHospitalityData() {
  console.log('🚀 Starting FIFA Hospitality Scraper...\n');
  ...
  try {
    ...
    console.log('📡 Navigating to FIFA website...');
    await page.goto('https://fifaworldcup26.hospitality.fifa.com/us/en/choose-matches', {
    ...
    const matches = await page.evaluate(async () => {
      ...
      const matchRes = await fetch('/next-api/matches-all?productCode=26FWC', {
        headers: { 'country-tag': 'us', 'language-tag': 'en' }
      });
        ...
        try {
          ...
          if (loungeRes.ok) {
            ...
            match.lounges = lounges.map(l => ({
              ...
              priceNumber: (() => {
                const match = (l.comparePrice || '').match(/\$([0-9,]+)/);
    ...
    const data = {
      ...
      matchesWithPricing: matchesWithPricing.length,
    ...
    const csvRows = ['Match Number,Host Team,Away Team,Venue,City,Country,Date,Time,Lounge Type,Price USD'];
